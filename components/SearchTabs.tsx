...
const SearchTabs: React.FC<SearchTabsProps> = ({ sessions, activeId, onSwitch, onClose, onNew }) => {
  return (
    <div className="flex items-center gap-2 overflow-x-auto no-scrollbar pb-2 w-full border-b border-slate-800/50 mb-2 shrink-0">
      {sessions.map((session) => (
        <div
          key={session.id}
          onClick={() => onSwitch(session.id)}
          className={`
            group relative flex items-center gap-2 min-w-[140px] max-w-[200px] px-4 py-2 rounded-t-lg cursor-pointer border-b-2 transition-all duration-200
            ${activeId === session.id 
